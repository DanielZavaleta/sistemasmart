        ...
        <div className="w-full max-w-7xl p-6 mx-auto">
            <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                ...
                {reporte && (
                    <button
                        onClick={handlePrint}
                        className="bg-gray-700 hover:bg-gray-600 text-white px-5 py-2.5 rounded shadow flex items-center transition-colors"
                    >
            ...
            <div className="bg-gray-800 p-6 rounded-lg shadow-xl mb-8 border border-gray-700">
                <div className="flex flex-col lg:flex-row gap-6 items-end">
                    <div className="w-full lg:w-1/4">
                        <label className="block text-gray-400 mb-2 text-xs uppercase font-bold">Fecha Inicial</label>
                        <input
                            type="date"
                            value={fechaInicio}
                    ...
                    <div className="w-full lg:w-1/4">
                        <label className="block text-gray-400 mb-2 text-xs uppercase font-bold">Fecha Final</label>
                        <input
                            type="date"
                            value={fechaFin}
                    ...
                    <div className="w-full lg:w-1/4">
                        <label className="block text-gray-400 mb-2 text-xs uppercase font-bold">Cajeros</label>
            ...
            {reporte && (
                <div className="space-y-8 animate-fade-in-up">
                    ...
                    <div className="bg-gray-800 rounded-lg shadow-xl overflow-hidden border border-gray-700">
                        <h3 className="text-lg font-bold text-white p-4 bg-gray-900 border-b border-gray-700 flex items-center">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="bg-gray-900 text-gray-400 uppercase text-xs font-semibold">
                                    <tr>
                                        <th className="p-4 border-b border-gray-700">Nombre Cajero</th>
                                        <th className="p-4 border-b border-gray-700 text-center">Fecha Inicial</th>
                                        <th className="p-4 border-b border-gray-700 text-center">Fecha Final</th>
                                        <th className="p-4 border-b border-gray-700 text-center">Hora Inicial</th>
                                        <th className="p-4 border-b border-gray-700 text-center">Hora Final</th>
                                        <th className="p-4 border-b border-gray-700 text-right">Total Ventas</th>
                                    </tr>
                                </thead>
                                    ...
                                    {reporte.ventas_por_cajero && reporte.ventas_por_cajero.length > 0 ? (
                                        reporte.ventas_por_cajero.map((cajero, index) => (
                                            <tr key={index} className="hover:bg-gray-700/50 transition-colors">
                                                ...
                                                <td className="p-4 text-center text-gray-300">{formatDate(cajero.fecha_inicial)}</td>
                                                <td className="p-4 text-center text-gray-300">{formatDate(cajero.fecha_final)}</td>
                                                <td className="p-4 text-center text-gray-300 font-mono">{formatTime(cajero.fecha_inicial)}</td>
                                                <td className="p-4 text-center text-gray-300 font-mono">{formatTime(cajero.fecha_final)}</td>
                                                <td className="p-4 text-right font-bold text-cyan-400 text-lg">${cajero.total_ventas.toFixed(2)}</td>
                                            </tr>
                                        ))
                                    ) : (
                                        <tr>
                                            <td colSpan="6" className="p-8 text-center text-gray-500 italic">No se encontraron ventas para este período.</td>
                                        </tr>
                                    )}
                                </tbody>
                                <tfoot>
                                    <tr className="bg-gray-900/80 font-bold text-white">
                                        <td colSpan="5" className="p-4 text-right uppercase text-xs tracking-wider text-gray-500">Total General</td>
