...
class ConfiguracionViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        # If user asks for list explicitly, default behavior, BUT
        # for a singleton config, let's treat LIST as GET_INSTANCE mostly,
        # or just normal CRUD.
        # Let's keep normal CRUD but help frontend create one if empty.
...
class CorteCajaViewSet(viewsets.ModelViewSet):
    ...
    def calcular_totales(self, request):
        ...
        ventas_hoy = Venta.objects.filter(creado_en__range=(today_start, today_end))
        ...
        # Restar Salidas de Efectivo (Pagos a Proveedores)
        pagos_prov_hoy = PagoProveedor.objects.filter(fecha__range=(today_start, today_end), metodo='efectivo')
        total_pagos_prov = pagos_prov_hoy.aggregate(Sum('monto'))['monto__sum'] or 0
        
        # Restar Retiros de Caja (HU-17)
        retiros_hoy = RetiroCaja.objects.filter(fecha__range=(today_start, today_end))
        total_retiros = retiros_hoy.aggregate(Sum('monto'))['monto__sum'] or 0

        # Abonos
        abonos_hoy = MovimientoCliente.objects.filter(fecha__range=(today_start, today_end), tipo='abono')
